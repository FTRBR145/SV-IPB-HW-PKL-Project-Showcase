    ...
    <section id="about" className="py-16 bg-white border-b border-slate-200">
      <div ref={sectionRef} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid md:grid-cols-2 gap-10 items-center">
          <div>
            <h2
              className={`font-heading text-2xl sm:text-3xl font-extrabold text-slate-800 mb-4 leading-tight title-underline-reveal scroll-reveal ${isVisible ? 'is-visible' : ''}`}
            >
              TRK Student Project Showcase
            </h2>
            {/* fade-left: slides in from the left alongside the heading */}
            <p
              className={`text-slate-600 text-sm sm:text-base leading-relaxed scroll-reveal scroll-reveal--left reveal-delay-2 ${isVisible ? 'is-visible' : ''}`}
            >
          ...
          <div
            className={`relative rounded-2xl overflow-hidden shadow-xl border border-slate-200/80 group scroll-reveal scroll-reveal--right reveal-delay-1 ${isVisible ? 'is-visible' : ''}`}
          >
            <picture>

...
export default function AdminSidebar({ activeMenu, onSelectMenu, onBack }) {
  ...
  return (
    <>
      <aside className="workspace-sidebar hidden w-52 flex-shrink-0 select-none flex-col overflow-y-auto border-r p-3 md:sticky md:top-0 md:flex md:h-[calc(100dvh-5rem)] md:self-start">
          ...
          {ADMIN_MENU.slice(0, 5).map(({ icon: Icon, label, id }) => {
            ...
            return (
              <button
                ...
                className={`flex min-h-11 w-full items-center justify-between rounded-lg px-3 py-2.5 text-xs font-bold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500 ${
                  active
        ...
        <div className="py-3 border-b border-slate-100 space-y-1">
          <span className="mb-1 block px-3 text-xs font-semibold text-slate-600">
            Manajemen Sistem
          </span>
          {ADMIN_MENU.slice(5).map(({ icon: Icon, label, id }) => {
            ...
            return (
              <button
                ...
                aria-current={active ? 'page' : undefined}
                onClick={() => onSelectMenu(id)}
                className={`flex min-h-11 w-full items-center gap-2.5 rounded-lg px-3 py-2 text-xs font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500 ${
                  active
        ...
        <div className="pt-3 space-y-2 mt-auto">
          ...
          <button
            onClick={onBack}
            className="flex min-h-11 w-full items-center justify-center gap-2 rounded-lg bg-slate-100 px-3 py-2 text-xs font-bold text-slate-700 transition-colors hover:bg-slate-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500"
          >
      ...
      >
            ...
            <button
              type="button"
              onClick={onBack}
              className="mt-3 flex min-h-11 w-full items-center justify-center gap-2 rounded-lg bg-slate-100 px-3 py-2.5 text-xs font-bold text-slate-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500"
            >
              <ArrowLeft size={14} /> Kembali ke Website
            </button>
      </ModalShell>
        ...
        <div className="mx-auto grid min-h-16 max-w-lg grid-cols-4 items-stretch py-1.5">
          {primaryMenu.map(({ icon: Icon, label, id }) => {
            ...
            return (
              ...
              >
                <span className={`rounded-lg p-1.5 ${active ? 'mobile-nav-active bg-sky-100' : ''}`}><Icon size={18} /></span>
          ...
          >
            <span className={`rounded-lg p-1.5 ${isMoreOpen || secondaryActive ? 'mobile-nav-active bg-sky-100' : ''}`}><MoreHorizontal size={18} /></span>
            <span>Lain</span>
          </button>
        </div>
      </nav>
